#!/bin/sh
# tools/fixtures/d/document_mirror_control.sh -- the mirror check, proven from both sides.
#
...
#
# USAGE
#   sh tools/fixtures/d/document_mirror_control.sh
#
# Driven by tools/d/document_mirror_witness.rish. Run from the repository root.

set -u

SCAN=$(pwd)/tools/fixtures/d/document_mirror_scan.sh
faults=0
pen=$(mktemp -d)
