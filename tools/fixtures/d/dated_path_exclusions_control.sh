#!/bin/sh
# tools/fixtures/d/dated_path_exclusions_control.sh -- the single-list rule, proven both ways.
#
...
real=0
for f in "$root/tools/fixtures/d/dated_path_scan.sh" "$root/tools/fixtures/d/dated_path_repoint_scan.sh"; do
  n=$(detect "$f")
  real=$((real + n))
...
cp "$root/tools/fixtures/d/dated_path_scan.sh" "$work/private.sh"
...
cp "$root/tools/fixtures/d/dated_path_exclusions.sh" "$dpen/tools/fixtures/d/"
...
d_out=$( cd "$dpen" && . tools/fixtures/d/dated_path_exclusions.sh && dp_discovered_fixture_basenames . )
has() { printf '%s\n' "$d_out" | grep -qxF -- "$1"; }
...
printf 'DP_X="20260101-000000_listed-only.md"\n' >> "$dpen/tools/fixtures/d/dated_path_exclusions.sh"
( cd "$dpen" && git add -A && git -c user.email=p@p -c user.name=p commit -qm add ) >/dev/null 2>&1
d2=$( cd "$dpen" && . tools/fixtures/d/dated_path_exclusions.sh && dp_discovered_fixture_basenames . )
