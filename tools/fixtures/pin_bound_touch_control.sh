#!/bin/sh
# tools/fixtures/pin_bound_touch_control.sh -- the pen for pin_bound_touch_scan.sh.
#
...
#
#   sh tools/fixtures/pin_bound_touch_control.sh
#
# Run from the repository root; the pen is removed on exit whether it passes or fails.
set -u

scan=tools/fixtures/pin_bound_touch_scan.sh
reader=tools/fixtures/living_pin_max_bytes.sh
...
run()      { ( cd "$pen" && sh tools/fixtures/pin_bound_touch_scan.sh "$@" 2>/dev/null ); }
verdict()  { run "$@" | grep '^verdict=' | head -1 | cut -d= -f2; }
key()      { k=$1; shift; run "$@" | grep "^$k=" | head -1 | cut -d= -f2; }
exits()    { ( cd "$pen" && sh tools/fixtures/pin_bound_touch_scan.sh "$@" >/dev/null 2>&1 ); echo $?; }
