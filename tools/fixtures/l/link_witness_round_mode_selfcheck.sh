...
LINK_WITNESS_SNAPSHOT="$SNAP" sh tools/fixtures/l/link_witness_scan.sh >/tmp/lw_snap.out
test -f "$SNAP" || { echo "FAIL: snapshot file missing"; exit 1; }

echo "round_selfcheck: COMPARE equal (must GREEN)…"
if ! LINK_WITNESS_COMPARE="$SNAP" sh tools/fixtures/l/link_witness_scan.sh >/tmp/lw_cmp_ok.out; then
...
LINK_WITNESS_ALLOW_BASELINE=1 LINK_WITNESS_COMPARE="$SNAP" \
  sh tools/fixtures/l/link_witness_scan.sh >/tmp/lw_cmp_red.out 2>&1
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
