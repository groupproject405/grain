#!/bin/sh
# tools/fixtures/r/reds_row_present_control.sh -- prove the spine reading on a pen, both directions.
#
...
#
# BOTH READINGS, because both were the same fault. tools/fixtures/r/reds_row_present.sh answers for a
# ROW NUMBER and tools/fixtures/r/reds_spine_grep.sh answers for a LESSON'S TEXT, and the season scans
# asked the living pin for each of them on adjacent lines.
#
#   sh tools/fixtures/r/reds_row_present_control.sh
#
# Exit 0 when all thirteen hold, 1 naming the first that did not. Touches nothing in the tree.
set -eu

SCAN=tools/fixtures/r/reds_row_present.sh
pen=$(mktemp -d) || exit 1
...
grep_check() {
  name=$1; want=$2; shift 2
  set +e
  REDS_SPINE_GLOB="$GLOB" sh tools/fixtures/r/reds_spine_grep.sh "$@" >/dev/null 2>&1
  got=$?
  set -e
  if [ "$got" -ne "$want" ]; then
    echo "CONTROL_FAIL case=${name} want_exit=${want} got_exit=${got}"
