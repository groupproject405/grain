#!/bin/sh
# tools/fixtures/r/readme_metrics_control.sh -- a drifted block, so the check is proven to notice.
#
# WHY. tools/fixtures/r/readme_metrics_check.sh refuses a stale metrics block. A check that has
...
code=0
sh tools/fixtures/r/readme_metrics_check.sh "$work/README.md" > "$work/out" 2>&1 || code=$?
