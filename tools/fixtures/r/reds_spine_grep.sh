#!/bin/sh
# tools/fixtures/r/reds_spine_grep.sh -- does the REDS ledger record this text, anywhere in its spine?
#
...
# the row-number half in tools/fixtures/r/reds_row_present.sh, one line further down each scan: a fold
# repoints documents and leaves guards standing, because a guard's citation is code rather than a
# link (REDS %231).
#
# The spine's file set is spelled once, in tools/fixtures/r/reds_spine_files.sh, and this asks that
# script for it rather than repeating the glob.
#
#   sh tools/fixtures/r/reds_spine_grep.sh 'a pattern'        # exit 0 when the spine records it
#   sh tools/fixtures/r/reds_spine_grep.sh -i 'a pattern'     # case-insensitively
#   sh tools/fixtures/r/reds_spine_grep.sh -i 'pat' --say     # and print which file recorded it
#
...
files=$(sh tools/fixtures/r/reds_spine_files.sh) || {
