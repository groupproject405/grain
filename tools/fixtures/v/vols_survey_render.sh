...
classified=$(sh tools/fixtures/v/vols_classify_scan.sh "$root" "$guardlist" 2>/dev/null)

surfaces=$(sh tools/fixtures/v/vols_survey_scan.sh "$root" 2>/dev/null | sed -n 's/^surfaces=//p')
