#!/bin/sh
# tools/fixtures/v/vols_survey_readtrue.sh -- VOLS survey, the ledger read true (r4's crux).
#
#   sh tools/fixtures/v/vols_survey_readtrue.sh <root> <guardlist>
#
...
ledger=$(sh tools/fixtures/v/vols_survey_render.sh "$root" "$guardlist" 20260101.000000 2>/dev/null)
l_sites=$(printf '%s\n' "$ledger" | sed -n 's/^sites //p')
l_handles=$(printf '%s\n' "$ledger" | sed -n 's/^handles //p')
l_prose=$(printf '%s\n' "$ledger" | sed -n 's/^prose_gaps //p')

truth=$(sh tools/fixtures/v/vols_survey_truth.sh "$root" "$guardlist" 2>/dev/null)
