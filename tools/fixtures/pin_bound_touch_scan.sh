#!/bin/sh
# tools/fixtures/pin_bound_touch_scan.sh -- a round that moves a living pin measures it before it ships.
#
# WHAT THIS IS FOR. Seven pages in this tree are living pins, rostered in
# tools/fixtures/living_pin_guard_roster.txt, and each carries a byte bound so an agent can read it
...
# one bound reading (tools/fixtures/living_pin_max_bytes.sh, REDS %199) -- so the two roofs cannot
...
#
#   sh tools/fixtures/pin_bound_touch_scan.sh                  # the index -- what this commit ships
#   sh tools/fixtures/pin_bound_touch_scan.sh head             # what HEAD's own commit shipped
#   sh tools/fixtures/pin_bound_touch_scan.sh worktree         # every rostered pin, off disk
#   sh tools/fixtures/pin_bound_touch_scan.sh prove-red        # the planted refusal
#   sh tools/fixtures/pin_bound_touch_scan.sh staged --roster <path>   # the pen's own roster
#
...
#
# Proven both ways by tools/fixtures/pin_bound_touch_control.sh on real git repositories in a
