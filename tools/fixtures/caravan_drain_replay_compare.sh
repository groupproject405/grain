#!/bin/sh
# tools/fixtures/caravan_drain_replay_compare.sh -- two region snapshots, compared byte for byte.
#
...
if [ -z "$a" ] || [ -z "$b" ] || [ ! -d "$a" ] || [ ! -d "$b" ]; then
  echo "usage: sh tools/fixtures/caravan_drain_replay_compare.sh DIR_A DIR_B (both existing)"
  exit 2
fi
