#!/bin/sh
# tools/fixtures/c/crypto_tool_declaration_scan.sh -- a declared crypto tool is a redirect, not an excuse.
#
...
#
# The repair declares the second class in tools/fixtures/c/crypto_tool_modules.txt. This scan is what
...
#
# USAGE
#   sh tools/fixtures/c/crypto_tool_declaration_scan.sh          # key=value census
#   sh tools/fixtures/c/crypto_tool_declaration_scan.sh list     # one fault per line
#
# Driven by tools/cr/crypto_count_guard_witness.rish. Proven both ways by
# tools/fixtures/c/crypto_tool_declaration_control.sh. Run from the repository root.
set -eu

MODE="${1:-census}"
DECL="${CRYPTO_TOOL_DECL:-tools/fixtures/c/crypto_tool_modules.txt}"
