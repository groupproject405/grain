#!/bin/sh
# tools/fixtures/mind_source_adaptation_control.sh -- prove MIND adaptation gates both ways.
set -eu
...
check() {
  ...
  want=$2
  shift 2
  out=$(sh tools/fixtures/mind_source_adaptation_scan.sh --decide "$@" 2>&1 || true)
  case "$out" in
