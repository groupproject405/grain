#!/bin/sh
# tools/fixtures/f/fetch_toolchain_control.sh -- the refusal proven, without a 55 MB download.
#
# WHY. tools/fixtures/f/fetch_toolchain_scan.sh decides, on one comparison, whether 55 megabytes of
...
run_verify() {
  code=0
  out=$(sh "$root/tools/fixtures/f/fetch_toolchain_scan.sh" verify "$1" "$2" 2>/dev/null) || code=$?
...
good_ok=$(sh "$root/tools/fixtures/f/fetch_toolchain_scan.sh" verify "$work/good.bin" "$expect" >/dev/null 2>&1 && echo yes || echo no)
bad_ok=$(sh "$root/tools/fixtures/f/fetch_toolchain_scan.sh" verify "$work/bad.bin" "$expect" >/dev/null 2>&1 && echo yes || echo no)
absent_ok=$(sh "$root/tools/fixtures/f/fetch_toolchain_scan.sh" verify "$work/nothing-here.bin" "$expect" >/dev/null 2>&1 && echo yes || echo no)
