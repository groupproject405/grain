...
over70=0
if [ -x tools/fixtures/t/tame_style_long_fn_roster.sh ] && [ -x tools/fixtures/t/tame_style_long_fn_one.sh ]; then
  over70="$(
    sh tools/fixtures/t/tame_style_long_fn_roster.sh 2>/dev/null | while IFS= read -r f; do
      [ -n "$f" ] || continue
      sh tools/fixtures/t/tame_style_long_fn_one.sh "$f"
    done | wc -l | tr -d ' '
  )"
  over70="${over70:-0}"
fi
