#!/bin/sh
# fleet_clockout_control.sh -- prove the hand's drain, from both sides, on a real loop body.
#
...
stub() {
  ...
  while [ "$laps" -lt "$max" ]; do
    if [ -f "$pen/.loop-clockout" ]; then
      out="$out|clockout_before_lap_$((laps+1))"; break
    fi
    rm -f "$pen/.loop-gates-only"
    laps=$((laps+1))
    out="$out|lap_$laps"
    [ -f "$pen/mid" ] && touch "$pen/.loop-clockout"
    if [ -f "$pen/.loop-gates-only" ]; then out="$out|gates_only"; break; fi
    if [ -f "$pen/.loop-clockout" ]; then out="$out|clockout_after_lap_$laps"; break; fi
  done
  echo "$out"
}
...
touch "$pen/.loop-clockout"
r=$(stub incense 3)
case "$r" in *clockout_before_lap_1*) ok "a standing drain stops before a lap opens" ;; *) bad "pre-lap drain: $r" ;; esac
# `*lap_1*` matched `clockout_before_lap_1` when this leg was first written -- a substring
...
touch "$pen/mid"
r=$(stub incense 5)
case "$r" in *clockout_after_lap_1*) ok "a mid-lap drain stops after the lap finishes whole" ;; *) bad "mid-lap drain: $r" ;; esac
rm -f "$pen/mid" "$pen/.loop-clockout"

# 5 -- THE REGRESSION THIS EXISTS FOR: the loop never removes a drain
touch "$pen/.loop-clockout"
stub incense 2 >/dev/null
if [ -f "$pen/.loop-clockout" ]; then ok "the loop leaves the drain for the hand that set it"
...
grep -q 'loop-drain' "$root/grain-incense/tools/f/fleet-loop.sh" 2>/dev/null \
  ...
  && ok "fleet-loop.sh reads .loop-clockout" || bad "fleet-loop.sh reads no drain"
