...
if [ "$(reading "$clean" verdict)" = "ok" ] \
  && [ "$(reading "$clean" fresh_header)" = "mantra-weave-20260916.101910" ] \
...
v1=$(run_pen writes_v1 's|try out.appendSlice(allocator, "mantra-weave-20260916.101910\\n");|try out.appendSlice(allocator, "mantra-weave-v1\\n");|')
...
# own reader.
d3=$(run_pen drops_stamped_read 's|^.*if (std.mem.eql(u8, header, "mantra-weave-20260916.101910")) return read_order_record.*$||')
echo "drops_stamped_read_mixed_opens=$(reading "$d3" mixed_opens)"
