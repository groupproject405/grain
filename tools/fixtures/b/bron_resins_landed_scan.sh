#!/bin/sh
# tools/fixtures/b/bron_resins_landed_scan.sh -- a resin's bytes are the bytes that landed.
#
# WHAT THIS READS. `bron-resins/` is the resin cellar, a room of chat printouts written once on
# `20260712` and kept as testimony. `bron-resins/manifest.bron` is its catalog, and the paragraph
...
#
# WHY IT IS A SEPARATE READING FROM THE SEAL. `bron_resins_catalog_scan.sh` recomputes each resin's
...
#
#   sh tools/fixtures/b/bron_resins_landed_scan.sh [<room>] [--list]
#
# Default room `bron-resins`. Exit 0 when every resin stands as it landed, 1 when one has moved,
...
echo "room=$ROOM"
echo "catalog=$ROOM/manifest.bron"

TMP=$(mktemp -d "${TMPDIR:-/tmp}/bron_resins_landed.XXXXXX") || exit 2
trap 'rm -rf "$TMP"' EXIT

# The room's own resins: every regular file directly in it, the living catalog aside.
find "$ROOM" -maxdepth 1 -type f ! -name manifest.bron | LC_ALL=C sort > "$TMP/files"
