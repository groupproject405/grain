#!/bin/sh
# tools/fixtures/b/bron_resins_landed_control.sh -- prove bron_resins_landed_scan.sh on real git history.
#
...
#   sh tools/fixtures/b/bron_resins_landed_control.sh
set -u

ROOT=$(pwd)
SCAN="$ROOT/tools/fixtures/b/bron_resins_landed_scan.sh"
PEN=$(mktemp -d "${TMPDIR:-/tmp}/bron_resins_landed_control.XXXXXX") || exit 2
trap 'rm -rf "$PEN"' EXIT
...
mkpen() { # mkpen <name> ; echoes its repository root
  d="$PEN/$1"
  mkdir -p "$d/bron-resins"
  git -C "$d" init -q 2>/dev/null
  git -C "$d" config user.email pen@example.invalid
  git -C "$d" config user.name pen
  git -C "$d" config commit.gpgsign false
  printf 'a\n' > "$d/bron-resins/20260101-000001_first.bron"
  printf 'b\n' > "$d/bron-resins/20260101-000002_second.bron"
  printf 'format bron-resins-v1\n' > "$d/bron-resins/manifest.bron"
  git -C "$d" add -A
...
capture() { # capture <repo> [<scan>] [<flag>] ; echoes the transcript path
  ...
  state=$( (git -C "$1" rev-parse HEAD 2>/dev/null; find "$1/bron-resins" -type f -exec cksum {} + 2>/dev/null | LC_ALL=C sort) | cksum | cut -d' ' -f1)
  key=$(printf '%s|%s|%s|%s' "$1" "${2:-$SCAN}" "${3:-}" "$state" | cksum | cut -d' ' -f1)
  out="$CACHE/$key"
  if [ ! -f "$out" ]; then
    ( cd "$1" && sh "${2:-$SCAN}" bron-resins ${3:-} 2>/dev/null ) > "$out" || true
  fi
  echo "$out"
}
...
exit_of() { # exit_of <repo> [<scan>]
  ( cd "$1" && sh "${2:-$SCAN}" bron-resins >/dev/null 2>&1 )
  echo $?
}

echo "bron-resins landed control -- real repositories in $PEN"
...
p=$(mkpen edited)
printf 'a changed\n' > "$p/bron-resins/20260101-000001_first.bron"
...
p=$(mkpen resealed)
printf 'a rewritten\n' > "$p/bron-resins/20260101-000001_first.bron"
printf 'seal 20260101-000001_first.bron deadbeef\n' >> "$p/bron-resins/manifest.bron"
git -C "$p" add -A
...
p=$(mkpen uncommitted)
printf 'c\n' > "$p/bron-resins/20260101-000003_third.bron"
...
p=$(mkpen catalog)
printf 'entry 20260101-000001_first.bron the first resin\n' >> "$p/bron-resins/manifest.bron"
git -C "$p" add -A
...
p=$(mkpen renamed)
mkdir -p "$p/bron-resins/date"
git -C "$p" mv bron-resins/20260101-000002_second.bron bron-resins/20260101-000002_second_moved.bron
...
p=$(mkpen readded)
git -C "$p" rm -q bron-resins/20260101-000001_first.bron
...
check "an unknown flag is a misread"  "misread" "$(cd "$p" && sh "$SCAN" bron-resins --nope 2>/dev/null | grep '^verdict=' | cut -d= -f2)"
mkdir -p "$PEN/bare/bron-resins"
printf 'x\n' > "$PEN/bare/bron-resins/20260101-000001_first.bron"
check "no repository is a misread"    "misread" "$(cd "$PEN/bare" && sh "$SCAN" bron-resins 2>/dev/null | grep '^verdict=' | cut -d= -f2)"

# --- the listing names its own drop (REDS %797) -------------------------------------------------
p=$(mkpen listing)
i=3
while [ "$i" -le 70 ]; do
  printf 'r%s\n' "$i" > "$p/bron-resins/20260101-0000$i""_bulk.bron"
  i=$((i + 1))
done
git -C "$p" add -A
...
m=$(mutate keep_catalog 's/ ! -name manifest\.bron//')
behaviors=$((behaviors + 1))
if [ -z "$m" ]; then
  ...
  p=$(mkpen mut_catalog)
  printf 'entry 20260101-000001_first.bron a note\n' >> "$p/bron-resins/manifest.bron"
  git -C "$p" add -A
  git -C "$p" commit -q -m "cellar: catalog a resin"
...
if [ -z "$m" ]; then
  ...
  check "mutation no_gate exits clean"  "0" "$(cd "$p" && sh "$m" bron-resins >/dev/null 2>&1; echo $?)"
fi
