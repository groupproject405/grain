...
# had no reader: three scans name `kyri-resins/` only to skip it as testimony, and no witness,
...
# `!/kyri-resins/`, so the room is allow-listed out of the root deny and a `find` read names the
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/kyri_resins_catalog.XXXXXX") || exit 2
trap 'rm -rf "$TMP"' EXIT

# The room's own resins: every regular file directly in it, minus the catalog.
find "$ROOM" -maxdepth 1 -type f ! -name manifest.kyri -exec basename {} \; \
