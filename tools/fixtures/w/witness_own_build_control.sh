...
( cd "$d" && i=2; while [ "$i" -le 19 ]; do printf 'let r = run ["bin/thing%s" "selftest"]\n' "$i" > "tools/spare${i}_witness.rish"; i=$((i + 1)); done
  git add -A && git commit -qm 'pen: nineteen unbuilt pairs' ) >/dev/null 2>&1
out=$(verdict_of "$d")
echo "$out" | grep -q 'unbuilt_pairs=19 ' && echo "ratchet_counted=yes" || echo "ratchet_counted=no"
echo "$out" | grep -q 'verdict=ok' && echo "ratchet_under_free=yes" || echo "ratchet_under_free=no"

( cd "$d" && printf 'let r = run ["bin/thing20" "selftest"]\n' > tools/spare20_witness.rish \
  && git add -A && git commit -qm 'pen: one over the ceiling' ) >/dev/null 2>&1
out=$(verdict_of "$d")
echo "$out" | grep -q 'unbuilt_pairs=20 ' && echo "ratchet_over_counted=yes" || echo "ratchet_over_counted=no"
