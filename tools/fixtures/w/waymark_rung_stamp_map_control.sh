...
awk -F '\t' 'BEGIN { OFS="\t" }
  ...
  { print }
' "$map" > "$bad_stamp"
if RUNG_MAP_PATH="$bad_stamp" /bin/bash "$scan" > "$pen_dir/bad-stamp.out" 2>&1; then
...
if RUNG_STOA_LEDGER="$bad_ledger" /bin/bash "$scan" > "$pen_dir/bad-ledger.out" 2>&1; then
...
awk -F '\t' 'BEGIN { OFS="\t" }
  !($1 == "STOA" && $2 == "346") { print }
' "$map" > "$missing"
if RUNG_MAP_PATH="$missing" /bin/bash "$scan" > "$pen_dir/missing.out" 2>&1; then
...
awk -F '\t' 'BEGIN { OFS="\t" }
  ...
  { print }
' "$map" > "$census"
RUNG_MAP_PATH="$census" /bin/bash "$scan" > "$pen_dir/census.out"
...
TZ=Pacific/Honolulu /bin/bash "$scan" > "$pen_dir/host-zone.out"
