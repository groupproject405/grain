#!/bin/sh
# tools/fixtures/reds_status_consistency_control.sh -- the pen that proves
# tools/fixtures/reds_status_consistency_scan.sh bites what it claims and frees what it claims.
#
...
# REDS_SPINE_GLOB, which tools/fixtures/reds_spine_files.sh honors, so the living ledger is never
# read here and never written.
#
#   sh tools/fixtures/reds_status_consistency_control.sh
#
# Exit 0 when every case agrees. Purely local: it writes markdown into a temporary directory and
# removes it.
set -eu

SCAN=tools/fixtures/reds_status_consistency_scan.sh
