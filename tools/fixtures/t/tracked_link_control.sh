#!/bin/sh
# tools/fixtures/t/tracked_link_control.sh -- prove the tracked-link meter can red, and where it stays free.
#
...
#
# USAGE
#   sh tools/fixtures/t/tracked_link_control.sh
#
# Driven by tools/t/tracked_link_witness.rish. Run from the repository root.

set -eu

scan="$(pwd)/tools/fixtures/t/tracked_link_scan.sh"
pen=$(mktemp -d)
trap 'rm -rf "$pen"' EXIT
