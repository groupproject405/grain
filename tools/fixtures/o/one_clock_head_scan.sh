...
#
# tools/fixtures/o/one_clock_head_erratum.txt pins the stamps already written
...
erratum=tools/fixtures/o/one_clock_head_erratum.txt
...
EOF
  ...
  head_line=$(sh tools/fixtures/o/one_clock_mono_scan.sh | sed -n 's/^TRUE_HEAD \(.*\)$/\1/p' | head -1)
  if [ -z "$head_line" ]; then
