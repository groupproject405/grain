#!/bin/sh
# tools/fixtures/reds_fold_control.sh -- prove the fold loom on planted trees, both directions.
#
...
#
#   sh tools/fixtures/reds_fold_control.sh
#
...
run_fold() {
  ...
  ( cd "$pen/t" && sh tools/fixtures/reds_fold.sh "$@" 2>&1 )
}
...
out=$(sh "$root/tools/fixtures/reds_fold_reanchor.sh" < "$pen/masked.txt" 2>&1) && rc=0 || rc=$?
