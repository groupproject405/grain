#!/bin/sh
# tools/fixtures/reds_status_consistency_scan.sh -- a row the ledger says is closed does not still
# read OPEN.
#
...
# still carries the bare uppercase word OPEN, which is the looser test tools/fixtures/reds_fold.sh
...
#
# THE SETS ARE SPELLED ONCE. The spine's file set comes from tools/fixtures/reds_spine_files.sh and
# the row shape is the same expression tools/fixtures/reds_row_present.sh reads. That the two agree
# is proven on metal by the witness rather than promised here: it reads this scan's highest row
# against tools/fixtures/reds_ledger_monotone_scan.sh's own expect_next, and asks
# reds_row_present.sh for that row and the one past it.
#
#   sh tools/fixtures/reds_status_consistency_scan.sh
#   REDS_SPINE_GLOB="pen/*.md" sh tools/fixtures/reds_status_consistency_scan.sh   # for a pen
#
