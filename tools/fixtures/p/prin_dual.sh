...
if command -v tmux >/dev/null 2>&1; then
  SESSION="grain-prin-$$"
  tmux new-session -d -s "$SESSION" "bash $ROOT/tools/fixtures/p/prin_matrix.sh $MATRIX_MODE"
  tmux split-window -h -t "$SESSION" "bash $ROOT/tools/fixtures/p/prin_ticker.sh slide"
...
exec bash "$ROOT/tools/fixtures/p/prin_matrix.sh" "$MATRIX_MODE"
