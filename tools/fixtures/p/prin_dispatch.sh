...
Prin (%prin) — Grain matrix printer · live outer-terminal view of loops & parity
  ...
  scope  | outer | inner   print seated season scope (six nests · tools/gen/chapter/prin_scope.rish)
...
EOF
  ...
  scope|outer|inner) exec "$ROOT/rishi/bin/rishi" run tools/gen/chapter/prin_scope.rish ;;
  sundial|psun) exec sh tools/fixtures/s/sundial.sh ;;
  ranking|rank|rankings|rota)
    ...
    echo ""
    echo "This round tends the slot for its round index; for a round's whole day: rishi/bin/rishi run tools/gen/chapter/itinerary.rish <r>"
    ;;
  ticker|pt|pv|slide) exec bash tools/fixtures/p/prin_ticker.sh slide ;;
  scroll) exec bash tools/fixtures/p/prin_ticker.sh scroll ;;
  dual|watch|pw|pd|p::) exec bash tools/fixtures/p/prin_dual.sh matrix ;;
  rain|pr|p\*) exec bash tools/fixtures/p/prin_matrix.sh rain ;;
  love|pl|p♥) exec bash tools/fixtures/p/prin_matrix.sh love ;;
  once|po|p.) exec bash tools/fixtures/p/prin_matrix.sh once ;;
  matrix|p|pm|px|*) exec bash tools/fixtures/p/prin_matrix.sh matrix ;;
esac
