...
#
#   sh tools/fixtures/e/equinox_e127_on_touch_scan.sh
#   sh tools/fixtures/e/equinox_e127_on_touch_scan.sh prove-red
#
...
ELDER_START=tools/fixtures/e/equinox_e126_start_rung_scan.sh
...
sh tools/fixtures/r/reds_row_present.sh 42 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_42"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'on-touch|campaign|ladder|foundations first' >/dev/null || {
  echo "reds=failed"
