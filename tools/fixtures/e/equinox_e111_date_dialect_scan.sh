...
#
#   sh tools/fixtures/e/equinox_e111_date_dialect_scan.sh
#
...
# tools/fixtures/d/date_dialect_scan.sh carries in full: the room grows, a label is what a new
...
rg -q 'Last updated' tools/fixtures/r/radiant_lint_scan.sh || {
  echo "lint_dep=failed"
  echo "verdict=misread"
  exit 1
}
# prove the scan still speaks (does not require zero advises -- only that it runs)
LINT_OUT=$(sh tools/fixtures/r/radiant_lint_scan.sh 2>&1 || true)
