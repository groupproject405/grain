...
for p in \
  "$HEALTH_SCAN" \
  tools/fixtures/f/fascia_health_live_control.md \
  tools/fixtures/2/20260731-150648_fascia_health_dated_control.md \
...
sh tools/fixtures/r/reds_spine_grep.sh -i 'on-disk is not in-the-tree' >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
