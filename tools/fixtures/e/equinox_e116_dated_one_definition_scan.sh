...
#
#   sh tools/fixtures/e/equinox_e116_dated_one_definition_scan.sh
#
...
ELDER=tools/equinox/witness/equinox_e115_instrument_suite_witness.rish
...
for p in \
  ...
  "$DIVERGE" \
  "$CLASSIFY" \
  tools/gen/chapter/dated_pattern_witness.rish \
  tools/gen/chapter/dated_roof_divergence_witness.rish \
  tools/fixtures/s/shed_census_scan.sh \
  tools/fixtures/f/fascia_health_scan.sh \
...
sh tools/fixtures/r/reds_row_present.sh 40 >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
  echo "detail=want_row_40"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'when two roofs carry one name' >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
