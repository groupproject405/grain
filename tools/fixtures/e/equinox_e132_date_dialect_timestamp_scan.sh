...
#
#   sh tools/fixtures/e/equinox_e132_date_dialect_timestamp_scan.sh
#   sh tools/fixtures/e/equinox_e132_date_dialect_timestamp_scan.sh prove-red
#
...
SAFE=SAFE.md
E111=tools/fixtures/e/equinox_e111_date_dialect_scan.sh
DD=tools/fixtures/d/date_dialect_scan.sh
...
# REDS 44
sh tools/fixtures/r/reds_row_present.sh 44 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_44"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'compact|timestamp|instrument|format' >/dev/null || {
  echo "reds=failed"
