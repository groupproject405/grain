...
#
#   sh tools/fixtures/e/equinox_e131_timing_pin_scan.sh
#   sh tools/fixtures/e/equinox_e131_timing_pin_scan.sh prove-red
#
...
# REDS row 43
sh tools/fixtures/r/reds_row_present.sh 43 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_43"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'timing|desk.tier|stopwatch|cache warmth' >/dev/null || {
  echo "reds=failed"
