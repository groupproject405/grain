...
for p in \
  "$THING_SCAN" \
  tools/fixtures/t/thing_not_name_emitter.sh \
...
sh tools/fixtures/r/reds_spine_grep.sh -i 'look for the thing, not for the name' >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
  echo "detail=want_thing_not_name_law"
  exit 1
}
sh tools/fixtures/r/reds_row_present.sh 38 >/dev/null || {
  echo "reds_row=failed"
  echo "verdict=misread"
