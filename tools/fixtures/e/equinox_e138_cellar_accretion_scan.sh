...
#
#   sh tools/fixtures/e/equinox_e138_cellar_accretion_scan.sh
#   sh tools/fixtures/e/equinox_e138_cellar_accretion_scan.sh prove-red
#
...
# REDS 48
sh tools/fixtures/r/reds_row_present.sh 48 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_48"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'accretion|outside consumer|complements' >/dev/null || {
  echo "reds=failed"
