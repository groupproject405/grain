...
#
#   sh tools/fixtures/e/equinox_e141_commence_amphora_scan.sh
#   sh tools/fixtures/e/equinox_e141_commence_amphora_scan.sh prove-red
#
...
# APPLY 1 -- REDS 50
sh tools/fixtures/r/reds_row_present.sh 50 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_50"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'timing figure is a pin|amphora_resin_chunk' >/dev/null "$COUNSEL" || {
  echo "reds=failed"
