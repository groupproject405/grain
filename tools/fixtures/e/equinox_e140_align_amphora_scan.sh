...
#
#   sh tools/fixtures/e/equinox_e140_align_amphora_scan.sh
#   sh tools/fixtures/e/equinox_e140_align_amphora_scan.sh prove-red
#
...
# REDS 49
sh tools/fixtures/r/reds_row_present.sh 49 >/dev/null || {
  echo "reds=failed"
  echo "detail=want_row_49"
  echo "verdict=misread"
  exit 1
}
sh tools/fixtures/r/reds_spine_grep.sh -i 'breach can be withdrawn|withdrawn' >/dev/null "$COUNSEL" || {
  echo "reds=failed"
