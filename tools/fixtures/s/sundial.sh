...
#
#   sh tools/fixtures/s/sundial.sh
#   sh tools/fixtures/s/sundial.sh shred   # RED refuse
set -eu
...
if grep -Eq 'innermost.*u0|MUR Chapter held at u0|innermost=paused\(u0\)' "$PROMPT"; then ok "prompt: innermost u0"; else miss "prompt: innermost u0"; fi
if grep -Eq 'core.*g0|Gren Chapter held at g0|core=paused\(g0\)' "$PROMPT"; then ok "prompt: core g0"; else miss "prompt: core g0"; fi
