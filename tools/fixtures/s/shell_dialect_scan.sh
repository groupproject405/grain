#!/bin/sh
# tools/fixtures/s/shell_dialect_scan.sh -- count GNU-only shell idioms in this tree's own guards.
#
...
#   sh tools/fixtures/s/shell_dialect_scan.sh prove-dialect  # reproduce the zero under a BSD xargs
#   sh tools/fixtures/s/shell_dialect_scan.sh prove-portable # a real scan reads the same under both
#
...
# sites moved to `tools/fixtures/s/shell_portable.sh` on `20260826.062128`, so the ceiling stands at
...
SELF='tools/fixtures/s/shell_dialect_scan.sh'
...
# `20260826.072239` under a BSD-shaped date: tools/fixtures/o/one_clock_provenance_scan.sh read two
...
# ones this tree leans on hardest: `tools/fixtures/d/dated_path_scan.sh`, whose lost-reference count
# gates every room fold with no slack, and `tools/fixtures/l/living_card_ascii_scan.sh`, which holds
...
# and has no `-c` at all. One site carried it, in `tools/fixtures/r/ryekey_control.sh`, and it carried
...
if [ "$mode" = prove-pcre ]; then
  ...
  # -- refuses it, so `tools/fixtures/d/dated_path_scan.sh` produced no pairs at all on that pier and
  ...
  #
  # THE CORPUS IS A PEN, and small on purpose. `tools/fixtures/d/dated_path_control.sh` already runs
...
SHIM
  ...
  sed -e 's/grep -rIoE/grep -rIoP/g' \
      ...
      tools/fixtures/d/dated_path_scan.sh > "$pen/tools/fixtures/planted_dated_path_scan.sh"
  cp tools/fixtures/d/dated_path_exclusions.sh "$pen/tools/fixtures/"
  ...
  live_gnu=$(read_total "$root/tools/fixtures/d/dated_path_scan.sh")
  live_bsd=$(PATH="$pen/bin:$PATH" read_total "$root/tools/fixtures/d/dated_path_scan.sh")
  ...
  echo "subject=tools/fixtures/d/dated_path_scan.sh:refs_total"
...
SHIM
  ...
  repaired_bsd=$(PATH="$pen/bin:$PATH" sh -c ". '$root/tools/fixtures/s/shell_portable.sh'; file_mtime '$subject'" 2>/dev/null | grep -c . || true)
...
if [ "$mode" = prove-date ]; then
  ...
  # found in: tools/fixtures/o/one_clock_provenance_scan.sh, which parses every dated stamp it weighs.
  #
...
ELDER
  ...
  ' tools/fixtures/o/one_clock_provenance_scan.sh > "$pen/tools/fixtures/planted_one_clock_provenance_scan.sh"
  ...
  cp tools/fixtures/s/shell_portable.sh "$pen/tools/fixtures/s/"

  read_ok() {
    ONE_CLOCK_PROVENANCE_BASE=HEAD ONE_CLOCK_PROVENANCE_STAMPS=20260101.120000 \
      sh "$1" 2>/dev/null | grep -c '^PROV_OK' || true
  }
  live_gnu=$(read_ok tools/fixtures/o/one_clock_provenance_scan.sh)
  live_bsd=$(PATH="$pen/bin:$PATH" read_ok tools/fixtures/o/one_clock_provenance_scan.sh)
  ...
  echo "subject=tools/fixtures/o/one_clock_provenance_scan.sh:PROV_OK"
...
if [ "$mode" = prove-portable ]; then
  ...
  sed "s|^xargs_lines_batched 400 \"\$work/living.txt\" .*|xargs -a \"\$work/living.txt\" -d '\\n' -n 400 \\\\|" \
    tools/fixtures/e/exec_bit_scan.sh > "$pen/tools/fixtures/planted_exec_bit_scan.sh"
  # The pen wears the root's two markers and the helper sits in its letter room, so the plant's
  # depth-proof walk (letter fold, seated 20260828) resolves the pen root and finds the helper.
  mkdir -p "$pen/rishi/bin" "$pen/tools/fixtures/s"
  cp tools/fixtures/s/shell_portable.sh "$pen/tools/fixtures/s/"
  ...
  live_gnu=$(read_invoked tools/fixtures/e/exec_bit_scan.sh)
  live_bsd=$(PATH="$pen/bin:$PATH" read_invoked tools/fixtures/e/exec_bit_scan.sh)
  ...
  echo "subject=tools/fixtures/e/exec_bit_scan.sh:directly_invoked"
  echo "living_gnu=${live_gnu:-none}"
  echo "living_bsd_shaped=${live_bsd:-none}"
  echo "planted_elder_gnu=${plant_gnu:-none}"
  echo "planted_elder_bsd_shaped=${plant_bsd:-none}"
