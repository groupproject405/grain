#!/bin/sh
# tools/fixtures/s/sow_lock_control.sh -- prove the projection lock from both sides.
#
#   sh tools/fixtures/s/sow_lock_control.sh        # all six behaviors (~171s)
#   sh tools/fixtures/s/sow_lock_control.sh fast   # the four refusals only (~1s)
#
# WHY. tools/fixtures/s/sow_project.sh clears seed/ and rebuilds it, so while it runs the
...
set +e
out=$(SEED_LOCK_DIR="$LOCK" sh tools/fixtures/s/sow_project.sh 2>&1)
code=$?
set -e
