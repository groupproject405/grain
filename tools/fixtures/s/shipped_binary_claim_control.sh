#!/bin/sh
# tools/fixtures/s/shipped_binary_claim_control.sh -- the guard proven to refuse, and to allow.
#
# WHY. tools/fixtures/s/shipped_binary_claim_scan.sh refuses a living document that says Grain's
...
run_scan() {
  code=0
  ( cd "$root" && sh tools/fixtures/s/shipped_binary_claim_scan.sh "$1" ) > "$work/out" 2>/dev/null || code=$?
...
run_scan "$work"
asserted_code=0
( cd "$root" && sh tools/fixtures/s/shipped_binary_claim_scan.sh "$work" ) >/dev/null 2>&1 || asserted_code=$?
...
run_scan "$work"
allowed_code=0
( cd "$root" && sh tools/fixtures/s/shipped_binary_claim_scan.sh "$work" ) >/dev/null 2>&1 || allowed_code=$?
