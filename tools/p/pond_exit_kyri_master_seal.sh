#!/usr/bin/env bash
# pond_exit_kyri_master_seal.sh -- exit kyri counts only when master-signed.
#
...
# custody-grade: detached pond-supersede-exit.kyri.asc must verify against
...
#   --season-closed  kyri and .asc absent - policy ready (scorecard sleep)
#   --require        kyri + .asc present - VALIDSIG from master fpr only
#
...
EXIT_KYRI="${REPO_ROOT}/kyri-resins/pond-supersede-exit.kyri"
EXIT_ASC="${EXIT_KYRI}.asc"
...
season_closed() {
  if [ -e "$EXIT_KYRI" ] || [ -e "$EXIT_ASC" ]; then
    echo "REFUSE: ${EXIT_KYRI} or .asc present while season claims closed — remove or claim exit properly" >&2
    return 1
  fi
  policy_ready
  echo "OK: season closed measured — exit kyri and .asc absent"
}

require_master_seal() {
  if [ ! -s "$EXIT_KYRI" ]; then
    echo "REFUSE: ENCLOSURE=pond while ${EXIT_KYRI} is absent — season still closed." >&2
    return 1
  fi
  if [ ! -s "$EXIT_ASC" ]; then
    echo "REFUSE: ${EXIT_ASC} absent — exit kyri counts only when master-signed." >&2
    return 1
  fi
  ...
  status="$(
    gpg --batch --trust-model always --status-fd 1 \
      --verify "${EXIT_ASC}" "${EXIT_KYRI}" 2>/dev/null || true
  )"
  ...
  echo "OK: exit kyri master-sealed · VALIDSIG ${MASTER_FPR}"
}
