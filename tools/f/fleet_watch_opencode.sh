#!/bin/sh
# fleet_watch_opencode.sh -- restart a stopped OpenCode loop when its seat is ready.
#
...
#
# This is the OpenCode twin of fleet_watch_codex.sh: it re-arms fleet-loop-opencode.sh
# rather than fleet-loop-codex.sh, and OpenCode runs bare by construction, so there is
# no FLEET_BARE branch to pass through.
#
# Usage:
#   sh tools/f/fleet_watch_opencode.sh                  # watch until stopped
#   sh tools/f/fleet_watch_opencode.sh --once           # one pass, then exit
#   sh tools/f/fleet_watch_opencode.sh --once --dry-run # report one pass; send no keys
#
...
#
# Hold checks:
#   - a live Claude, Codex, or OpenCode loop already owns the seat (%291)
...
cd "$root"
[ -f construction/ITINERARY.md ] || { echo "fleet-watch-opencode: $root is not a tree root"; exit 2; }

roster_scan=tools/fixtures/f/fleet_roster_scan.sh
[ -f "$roster_scan" ] || { echo "fleet-watch-opencode: missing $roster_scan -- the seat table is unreadable"; exit 2; }
...
# all got the captain's bench skipped anyway and no message saying so.
...
for arg in "$@"; do
  ...
  *) echo "fleet-watch-opencode: unknown option $arg"; exit 2 ;;
  esac
done
...
command -v tmux >/dev/null 2>&1 || { echo "fleet-watch-opencode: tmux is not on PATH -- nothing to watch"; exit 2; }
tmux has-session -t "$session" 2>/dev/null || { echo "fleet-watch-opencode: no tmux session named '$session'"; exit 2; }

say() { printf 'fleet-watch-opencode %s: %s\n' "$(TZ=America/New_York date +%H:%M:%S)" "$1"; }
...
loop_running() {
  ...
  # The trailing `\$` anchors end-of-string so an interior baton phrase never matches a dead loop.
  ...
  pgrep -f "fleet-loop-opencode\.sh $1\$" >/dev/null 2>&1
}
...
while :; do
  ...
  for seat in $live; do
    ...
    # NO PULL IN THE ARM LINE, on purpose. fleet_round_open.sh is the loop's first act and already
    # fetches, clears an interrupted rebase, stashes a dead lap's leavings, and adopts the anointed
    # order -- strictly more than the pull, and it cannot refuse the launch. OpenCode runs bare by
    # construction, so there is no enclosure flag to pass through.
    line="cd $tree && sh tools/f/fleet-loop-opencode.sh $seat"
    if [ "$dry" = 1 ]; then
