...
*Q3-Q6: version - bounds - CliError - couples - pour parse (`PourArgs`). Shared bounds agree via `tools/am/amphora_bounds_agree.rish`, which reads four relations -- a **derived** same-name roster, alias value groups read for membership before agreement, declared `couples:` equalities, and declared `covers:` orders. Run it for the counts; this line names the readings rather than their numbers. Pour/carry/restore wait their quests. Nested wave lean: **the Crossing Chapter** (seat Keaton's).*
...
**Ground:** silo [`foundations/20260703-201612_the-sealed-crossing.md`](../foundations/20260703-201612_the-sealed-crossing.md) - study [`external-research/date/20260703/20260703-201612_the-amphora-and-the-crossing.md`](../external-research/date/20260703/20260703-201612_the-amphora-and-the-crossing.md) - sealed crossing plainly [`external-research/date/20260710/20260710-002952_sealed-crossing-plainly.md`](../external-research/date/20260710/20260710-002952_sealed-crossing-plainly.md) - crossing metal plainly [`external-research/date/20260710/20260710-145313_amphora-crossing-plainly.md`](../external-research/date/20260710/20260710-145313_amphora-crossing-plainly.md)

**This door is read back, both ways.** `readme_unnamed` walks the roster and asks this page about every row it seats, and it has held at zero since `20260908`. The other direction waited until `20260911`. `readme_named` stood as `guards` minus `readme_unnamed` -- a number describing this page, derived wholly from the roster -- so the roster answered a question about this door on the door's own behalf. So a name this page writes and the roster has yet to seat stayed invisible to the instrument built to keep the two in step, which is how `amphora_device_wire` sat off the roster while this page named it. `readme_claims` counts what this door actually claims, and `readme_unrostered` names every claim still waiting on a roster row, under a ceiling that only falls. Its sibling one room over in this lane, `tools/fixtures/k/kyri_resins_catalog_scan.sh`, has read both directions since it was written -- `uncatalogued` beside `orphan_entries`, `unsealed` beside `orphan_seals`.
