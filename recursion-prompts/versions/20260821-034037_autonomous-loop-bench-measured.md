...
**The route did not move, and that is the finding.** An earlier pass this session called the loop's route stale for leading with the Microkernel Target rather than the newly-opened DISC arc. That was wrong and is booked as **REDS %104**: the Microkernel Target double-seat *takes precedence over Chapters A-H* on Keaton's seated `20260817` word, and Cryptography is Chapter G, inside A-H. The loops were carrying the seated ordering faithfully. **Both prompts keep their route exactly as written.**
...
`AMD 4vCPU/8GB, 180 GB NVMe, no swap.` The operator card had said `2vCPU/4GB` and was corrected in the same round; `GLOW_PROFILE.kyri` had it right all along.
