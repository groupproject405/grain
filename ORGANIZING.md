...
**`src/`** -- the Glow **userland**: code written in Glow, **three rooms**, each named for the word its own files already wear -- `shape/` the `shape-*.glow` desks (the data-shape museum), `gate/` the `gate-*.glow` desks (the shared arms many desks call), `gen/` the runnable generators. The six elder rooms were renamed on `20260827` ([the charter](active-designing/20260827-174816_the-glow-tree-and-the-rooms-that-say-what-they-hold.md)); `lib/` and `sur/` folded into `gate/` and `shape/`, and `app/`, `sec/` and `til/` keep their READMEs as pointers. The Glow *implementation* (tokenizer, runes, lowerers -- Rye) stays at `glow/`; the desks stay at `glow/gen/` while the O3 breach awaits Keaton's word (law in [`context/BREACH.md`](context/BREACH.md), state in [`src/gen/README.md`](src/gen/README.md)). Opened as an O1 skeleton `20260727.064038`.
