...
**Status:** Mixed -- Self-approved design round — opens the 1,024-itinerary's **Chapter 1 (The World, TACT), Journey 1**
...
BUHR closed the surface-and-intelligence equinox: the voices, the Realidream graph, the MCP surface, and Tablecloth all stand witnessed on metal. The intelligence and the surface met over one append-only graph. What no rung has done yet is let **a person** cross that whole floor from nothing — the itinerary's Chapter 1 durable promise: *a stranger reaches a working, signed, publishable instance without the maintainer's hand on each step.* The graph met the intelligence; now the graph meets a keeper.

Chapter 1 threads the **TACT** waymark (Compass Equinox 4, The World), already seated on the ladder. Its four journeys are Ship-Pilot · Publishing · Grainphone · Commerce. This round opens the first — **Ship-Pilot** — the guided first-run.
