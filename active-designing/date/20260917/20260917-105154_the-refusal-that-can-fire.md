...
**Row 3 erratum:** `20260917.121546` -- row 3 was ranked FIRST on *counters are readable today; gives every later row a unit*, and it ran. Its falsifier **did not fire**: over the pair this tree itself calls a pair -- `tools/fixtures/a/ascii_document_scan.sh` against `tools/fixtures/a/ascii_document_resident_probe.sh`, the same census with every per-file process removed -- five interleaved repetitions read `instructions_cheaper=resident`, `wall_cheaper=resident`, **`agree=yes`**. **The sharper half is where the two readings part.** They agree on ORDER and disagree on SIZE by **6.74x**: the counter reads the resident form doing **4.81x** less work (80,317,681,298 against 16,701,077,713 retired instructions) where the clock reads it taking **32.41x** less time (70.14s against 2.16s). The gap is the half the counter is built to exclude -- kernel time and scheduling latency for six thousand process spawns -- so the row's claim survives on ranking and its phrase *the same way a joule would* must never be read as magnitude. **And the steadiness holds at command scale:** instruction spread 33 and 1 ppm against wall spread 348,713 and 206,057 ppm, a clock 10,567x and 206,057x noisier than the counter on the same runs. The instrument is [`../tools/fixtures/r/retired_rank_scan.sh`](../tools/fixtures/r/retired_rank_scan.sh) over [`../tools/rye/retired_exec.rye`](../tools/rye/retired_exec.rye), witnessed by [`../tools/r/retired_rank_witness.rish`](../tools/r/retired_rank_witness.rish) at 19 behaviors with the DISAGREEING case proven on a sleeper-against-worker pair. The reading stands at [`20260917-121546_the-unit-that-agrees-on-order.md`](20260917-121546_the-unit-that-agrees-on-order.md). Recommended re-aim rather than re-rank: keep the row at rank one, and split its claim into the ranking half this reading measured and the magnitude half it never claimed. **Every figure in this clause is FREE** -- both arms read a tracked listing that grows every lap and the pier's load moves -- so run the scan rather than reading them. Every other word on this page is kept as written.
