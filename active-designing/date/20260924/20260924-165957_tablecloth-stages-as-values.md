...
This page names the reading and leaves the store unbuilt. A path can be renamed while the bytes stay put. A resin moves with the bytes. Two rooms that hold the same stage source arrive at the same name without agreeing on a directory. One changed line produces a different name, so a quiet edit shows up in the name itself.
