...
**Chapter A** (Hardware & Right-to-Repair) · **waymark DREY** · Mikrophone firmware journey · **rung DREY15**
...
`reconcile` moves bytes only between two in-memory catalogs, over the two proven loops' public APIs — no transport of its own. Running it between two real desks over a real network reaches the **Comlink-served custody gate** (Chapter 1, Journey 2) and waits for the maintainer's word. No disk, no network, no key signs, no funds — custody gate #2 untouched.
