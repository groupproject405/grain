...
**Status:** Mixed -- every **deep** entry (`[deep]`) below was written against its module source read whole in the workshop (rounds 1-3), with claims traceable to the STOA ledger; every **seed** entry (`[seed]`) restates the working pin ([`glow/README.md`](../../../glow/README.md)) and ledger ([`docs/STOA.md`](../../../docs/STOA.md)) -- none remain at v2. Anchors follow [`ANCHORS.md`](ANCHORS.md).
**Voice:** Quin (workshop)
...
<a id="g-triple"></a>
### [deep] `:+` triple - <a id="g-quad"></a>`:^` quad - <a id="g-list"></a>`:~` list
...
<a id="g-call"></a>
### [deep] `%-` call -- one argument

**Shape:** `%-  double  amount` -- gate, then one argument face, against a closed demo set. That line beside its `::` comment is the *entire* fixture [`../../../glow/gen/c/call-one.glow`](../../../glow/gen/c/call-one.glow) -- a desk is honestly small. **STOA6.** **Source:** [`rune_call.rye`](../../../glow/rune_call.rye) - [`lower_call.rye`](../../../glow/lower_call.rye) - [`tools/g/glow_run_desk_witness.rish`](../../../tools/g/glow_run_desk_witness.rish).

<a id="g-call2"></a>
### [deep] `%+` two-arg - <a id="g-call3"></a>`%^` three-arg - <a id="g-calln"></a>`%*` named-arg
...
<a id="g-cast"></a>
### [deep] `^-` cast -- say the shape at the boundary
...
<a id="g-shape"></a>
### [deep] `+$` name a shape - `$:` state fields - `$%` tag variants

**Shape** (a real desk, [`../../../glow/gen/s/shape-amount.glow`](../../../glow/gen/s/shape-amount.glow), whole):
...
<a id="g-switch"></a>
### [deep] `?-` switch - <a id="g-null"></a>`?~` null-test
...
<a id="g-assert"></a>
### [deep] `?>` assert-true - `?<` assert-false

**Shape:** `?>  (gth tick 32)  1` -- Hoon's wutgar/wutgal parallels, lowered to ordinary Rye `assert` (STOA88). TAME's first habit -- the design written where the machine can check it -- as a first-class rune pair, positive and negative space arriving together. **Source:** [`lower_assert.rye`](../../../glow/lower_assert.rye) - desk [`../../../glow/gen/a/assert-true.glow`](../../../glow/gen/a/assert-true.glow).

## The Binds -- `=` Holds

<a id="g-face"></a>
### [deep] `=/` face -- a typed let
...
<a id="g-mutate"></a>
### [deep] `=.` mutate - <a id="g-alias"></a>`=*` alias
...
<a id="g-barket"></a>
### [deep] `|^` barket -- the gate with a helper core
...
<a id="g-barlus"></a>
### [deep] `|+` barlus -- the fold that carries a value forward

**Shape:** a body line under a `|=` header, in two arities. The **range fold** names a verb, the header's own sample face, and a literal ceiling -- `|+  sumto  sample  8` folds `1..n` into one `@u32`, and `|+  prodto  sample  6` multiplies the same range. The **field fold** drops the ceiling, because a closed shape already carries its own count -- `|+  sum  xs` and `|+  prod  xs` reduce the fields of a `pair-shape` through `nona-shape`. **Refusals:** `MalformedBody` (an unknown verb, a missing ceiling, a ceiling of zero, or a decimal the house parse law declines) - `BodySampleMismatch`, since the folded face must be the header's own sample and never a stranger caught in passing - `SampleDoesNotNest`, which keeps the two arities apart, a range fold wanting bare `@u32` and a field fold wanting a closed tuple - and `FoldBoundTooLarge`, holding `prodto` at `max_prodto_bound = 12`, the largest factorial that fits `@u32`. An empty range answers `0` for a sum and `1` for a product, each its own identity; a range past its wall answers `0` by the fold-family law. **Desks:** [`src/gate/gate-tally-fold-sumto-u32.glow`](../../../src/gate/gate-tally-fold-sumto-u32.glow) and its six kin -- the first Glow Tend gates that carry a bounded value forward rather than deciding within a wall. **Ledger:** STOA332-336. **Source:** [`rune_shop_gate.rye`](../../../glow/rune_shop_gate.rye):`parse_body` - [`lower_shop_gate.rye`](../../../glow/lower_shop_gate.rye) - G1 row [`20260822-221639_glow-barlus-g1-row.md`](../../20260822-221639_glow-barlus-g1-row.md).

<a id="g-core"></a>
### [deep] `|%` core -- arms on a spine

**Shape:** `|%` - optional one `=/` payload line (bare `face  <decimal>` or typed `face=@u32  <decimal>`) - one to **three** `++` arms - `--`. A closed arm is a bare name; an open arm carries a body expression -- `sample` or the payload face, a literal, `(mix ...)` - `(jam ...)` - `(lent ...)` - `(cue ...)` - a `^-` cast, one-deep nest jams left/right/both -- and every face inside an open body is checked against the core's own scope (`BadBodyFace` otherwise): an arm may speak only of what the core actually holds. Order is law: `PayloadAfterArm` refuses a payload below the first arm; `TooManyPayloads`, `TooManyArms` name the bounds. Sixteen refusals in all. **Desk** ([`../../../glow/gen/c/core-double.glow`](../../../glow/gen/c/core-double.glow), whole): `|%` / `++  double` / `--`. **Ledger:** STOA14-27. **Source:** [`rune_core.rye`](../../../glow/rune_core.rye):`parse_source` - [`lower_core.rye`](../../../glow/lower_core.rye) - [`lower_compose_core*.rye`](../../../glow/lower_compose_core.rye).

<a id="g-lib"></a>
### [deep] `/+` library compose -- the import
