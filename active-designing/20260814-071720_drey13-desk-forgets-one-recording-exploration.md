...
**Chapter A** (Hardware & Right-to-Repair) · **waymark DREY** · Mikrophone firmware journey · rung **DREY13**
...
`forget` touches only the catalog's own store and index — no disk, no network, no key signs, no funds. Real hardware, a real desk, a real disk stay **custody gate #2**; serving or syncing over a real transport stays the **Comlink-served gate** (Chapter 1, Journey 2). This rung is pure Rye on the bench, bounded, invariant-asserted, refusing by name — the desk's forgetting made as readable, all the way down, as the device's.
