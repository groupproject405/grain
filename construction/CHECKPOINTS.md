...
**Checkpoint `20260906.212057` -- walk-back nib `e567cbc128`.** *Swept:* `construction/ITINERARY.md`, which stood at **40,976** of its 40,960 -- 16 over -- once this lap landed a parked round's Bakery account. *What was condensed:* the PETRICHOR block, whose `%512` detail this same lap folded to [`REDS-a-pen-one-path-wide-for-eight-ships-rows-512.md`](archive/REDS-a-pen-one-path-wide-for-eight-ships-rows-512.md) beside `%487` and `%508` *(link repointed `20260906.222500` -- the one shelf this cairn named was split into three, and `%487` landed on `REDS-a-handler-that-cleans-up-and-carries-on-rows-487.md`; every word of the cairn stands)*, and whose `%511` detail already stood on its own shelf. *What waits there, worth recalling:* the two-legs-two-failures reading that made `%512` a race rather than a flake, and the `66 -> 219 of 284` reach figure -- both whole in the shelves the condensed lines now point at. Petrichor's live question to Keaton -- 607 reachable pages, 82 leading nowhere -- is kept word for word. *Why a condense rather than a fold:* eleven cairns today already record this card at its ceiling, and the block condensed here is the one whose detail moved to a shelf in this very lap, so the pointer is newly true rather than newly thin.
