# ITINERARY -- pheromone's lower_face account, shelved whole

**Language:** EN
**Style:** Gauge, Meter setting
**Voice:** Kyri
**Status:** Shelf -- one landed account, immutable once written
**Room:** checkable -- the account of a landed lap, each claim carrying the witness that proved it
**Shelved:** `20260917.185254` from [`../ITINERARY.md`](../ITINERARY.md), to bring the card back
under its byte bound after a peer landing pushed it over.

The `glow/lower_face.rye` ident-stub lap, moved here whole to make room on the card. Its ask still
stands: whether the five remaining `usize` struct fields in `BoundValue` ride with the
`max_arm_len` three, or wait for the last lap in the arc.

**PHEROMONE -- THE LAST FILE AT ITS OWN BODY MD5, AND THE TWO LINES A STUB TOOK OFF THE METER.**
Elder account [shelved whole](20260917-153613_itinerary-pheromone-ident-account.md).
