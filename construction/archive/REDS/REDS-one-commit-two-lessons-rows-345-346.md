# REDS shelf -- rows %345, %346

**Language:** EN
**Folded:** `20260830.030609`, when the pin read 24,799 bytes against its declared 24,576
**Voice:** Kyri
**Status:** Shelf -- immutable once written; the living pin is [`../REDS.md`](../../REDS.md)

Two closed rows about one commit. `fa34b7a43` -- the interpreter wall raise of `20260829` --
taught a different lesson on each of its two faces, and the two were booked hours apart by the
same seat. %345 reads the commit's shape: a hook refusal was retyped without its `-- <paths>`
pathspec, and in that window a peer's staged lap was swept into the commit and pushed to both
remotes under the wrong message. The habit it seated is that the pathspec belongs on the send
rather than on the draft, because a refusal-and-retype is a fresh command crossing the shared
index again. %346 reads the same commit's words: it carried the retired *child-stream* into its
subject, its body, and two fresh comments, written by the seat that had itself seated the
dependent molt. The word arrived by inheritance from the text under edit, which is exactly when
a banner is weakest -- so the sweep-on-touch duty covers the words a lap copies forward, not
only the words it invents.

Both were caught by reading the pushed commit rather than by any standing guard, which is the
third thing the pair says together: a commit is the last artifact a lap produces and the first
one a reader meets, and nothing in the tree reads it after it lands except a person.
