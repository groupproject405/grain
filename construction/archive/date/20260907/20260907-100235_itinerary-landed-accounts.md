# Landed accounts -- folded from the live front `20260907.100235`

*Held word for word from [`../ITINERARY.md`](../../../ITINERARY.md), so the card carries what is OPEN and
what waits on Keaton's word. PHEROMONE's compile-reach account, folded on the round that booked the
doorway's unread rooms. Its relative links are re-anchored one level deeper here, which is the
correction a fold owes every link it moves. The question it left with you -- may
`index_shelf_repair.sh` drop a byte-identical duplicate row -- rides forward on the card.*

---

**PHEROMONE -- A GREP WAS READ AS A BUILD, AND THE LAP FINDING IT DIED TWICE.**
(`20260907.083054`) BOOKED, [folded](../../REDS/REDS-a-grep-that-was-read-as-a-build-rows-554.md).
...
[folded](20260907-083054_itinerary-landed-accounts.md).
