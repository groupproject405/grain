# Landed accounts -- folded from the live front `20260907.122532`

*Held word for word from [`../ITINERARY.md`](../../../ITINERARY.md), so the card carries what is OPEN and
what waits on Keaton's word. PHEROMONE's derived-runner account, folded on the round that took the
one seam it left named. Its relative links are re-anchored one level deeper here, which is the
correction a fold owes every link it moves.*

---

**PHEROMONE -- A RATCHET WHOSE ONLY REPAIR IS THE DEFECT IS A RATCHET NOBODY WORKS.**
Row `20260907.110022` **BOOKED**;
[folded](REDS-a-ratchet-whose-repair-is-the-defect-rows-565.md). `%532` left
...
where a non-exiting handler RESUMES (`%487`). **Three faults, three guards.** **That lap's named seam --
`glow_desk_reach_scan.sh` reading one instrument of two -- is taken above.** **Cold 169/164, 2 red (one root).**

**Prior account** [folded](20260907-104723_itinerary-landed-accounts.md).
