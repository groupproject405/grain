# ITINERARY landed accounts -- `20260907.001653`

**Language:** EN
**Stamp:** `20260907.001653`
**Status:** Shelf -- immutable once written
**Voice:** Kyri
**Folded from:** [`../ITINERARY.md`](../../../ITINERARY.md), the Diffuser account, when the card stood at
**40,906** bytes of a 40,960 bound -- 54 bytes of headroom against a replacement account of 1,632,
so the lap that follows it had nowhere to write until this section left.

**Re-measured on the fold that actually ran.** The lap that wrote this shelf died at its own commit
and the round-open stashed it; the lap that recovered it found the card moved by four peer commits,
so the byte figures above are this tree's rather than that one's -- 40,906 here where the elder
draft read 40,952, and 40,896 after the replacement rather than 40,917. Every word of the account
below is unchanged; only the two numbers describing the fold were taken again, because a shelf
written today should say what today measured.

**Every claim below landed.** The paper, its witness, its scan and its control stand in the tree at
`external-research/20260906-195719_what-it-costs-to-decide-the-next-hop.md` and
`tools/{t,fixtures/t}/topology_routing_*`; the `BPEN` line carried from Petrichor was repaired in
the same commit; and the fifteenth-firing question is a duplicate of the one merged onto the card
at `20260906.212057`, which stays live there. A finished section folds to a shelf. Every word below
is the card's own, unedited.
