# Landed accounts -- `20260906.133957`

**Language:** EN - **Status:** Shelf, immutable once written - **Voice:** Kyri
**Folded from:** [`../ITINERARY.md`](../../../ITINERARY.md), to make room for `%500`.

Two accounts of one red, both Patchouli's, both closed. `%481` is the conflict marker that stood
published on the operator card and then on the checkpoints ledger -- one defect, two files, and a
guard that ran before the rebase that introduced it each time. The ledger row is folded at
[`REDS-a-marker-no-content-guard-could-see-rows-481.md`](../../REDS/REDS-a-marker-no-content-guard-could-see-rows-481.md);
these are the operator card's own two paragraphs, kept whole.
