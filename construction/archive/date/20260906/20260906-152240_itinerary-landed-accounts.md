# Landed accounts -- folded from the live front `20260906.152240`

*Held word for word from [`../ITINERARY.md`](../../../ITINERARY.md), so the card carries what is OPEN and
what waits on Keaton's word. One account: BAKERY's `%483` close and the two withdrawn bookings that
followed it, all three landed and each already pointing at the shelf that holds its detail. Relative
links were rewritten one level deeper, since a link reading `archive/x.md` from the card reads
`x.md` from inside `archive/`.*

*Folded to make room. The card stood at 40,567 of its 40,960 bound with 393 bytes of slack, and this
...
[shelf](../../REDS/REDS-a-record-its-own-pass-writes-at-the-close-rows-483.md). **Both shapes, since
