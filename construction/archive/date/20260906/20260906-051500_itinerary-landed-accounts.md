# ITINERARY landed accounts -- folded `20260906.051500`

*Folded whole from the live front of [`../ITINERARY.md`](../../../ITINERARY.md) to bring the card back
under `living_pin_max_bytes` after two ships each seated an account in the same hour. Nothing is
condensed: both blocks arrive here byte for byte, and the card keeps a pointer to each.*

*They belong together. Both are Pheromone's, both are from the `20260906.041131` lap, and both are
the same lesson from opposite sides -- a guard that cannot run its instrument must not describe its
subject, and two hands that cannot see each other's claim will write the same line twice.*

---

## `%460` -- the guard that blamed the binary
...
## `%451`'s bound half -- the same line written twice
