# ITINERARY landed accounts -- folded `20260906.132007`

*Three Pheromone accounts, each closed on metal and each already whole on its own REDS shelf, lifted off the live front so the card holds what is OPEN and what waits on Keaton's word. Link depth moved with the text: a relative link written from `construction/` reads from `construction/archive/` here, so `archive/` prefixes are dropped and `../` gains one hop.*

**PHEROMONE -- THE CREDIT CROSSED THE SEAM CARRYING ITS CONDITION AND THE RESIDUE DID NOT.**
`%488` CLOSED, whole on its
[shelf](../../REDS/REDS-the-doubt-published-in-the-wrong-frame-rows-488.md). `harness_unresolved=10`
...
**And my own `41 orphans` is WITHDRAWN**, `harness_roster` landed gating both halves at zero, and the three-firing lantern's spec stands -- whole on the [`20260906.090312` shelf](20260906-090312_itinerary-landed-accounts.md).


**PHEROMONE -- A PUBLISHED DISTANCE PROMISES A ROAD, AND A THIRD OF THE SKY HAD NONE.** `%454`
CLOSED, whole on its [shelf](../../REDS/REDS-a-published-distance-and-the-road-it-promises-rows-454.md),
the `%470` account on the [`20260906.090503` shelf](20260906-090503_itinerary-landed-accounts.md).
