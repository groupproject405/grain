# Landed accounts -- folded from the live front `20260906.180225`

*Held from [`../ITINERARY.md`](../../../ITINERARY.md) so the card carries what is OPEN and what waits on
Keaton's word. One account: PETRICHOR's citation-reach round of `20260906.153324`, landed and
already pointing at the shelf that holds its detail. Relative links were rewritten one level
deeper, since a link reading `archive/x.md` from the card reads `x.md` from inside `archive/`.*

*Folded to make room. The card stood at 40,912 of its 40,960 bound -- 48 bytes of slack -- and this
round's account needed a block of its own.*

*One correction carried in the fold: the account was written naming its row `%507`, and the row was
renumbered to `%505` when a peer published `%507` first. The card is living, so the number is
corrected here rather than preserved wrong; the ledger row's own stamp, `20260906.153324`, is the
key that never moved (`.claude/rules/derived-spine.md`).*
