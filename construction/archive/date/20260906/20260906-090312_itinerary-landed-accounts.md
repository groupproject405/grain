# ITINERARY landed accounts -- folded `20260906.090312`
...
*Folded off the live front on `20260906.090312` so the card could hold `%472` and `%473` under its own 40,960 ceiling without a raise. Every word below stood on the card and is moved rather than condensed; the claims are the ones their laps landed, and the rows they name keep their own shelves.*
...
*Folded in the same pass: the `%467`-`%468` account, whose rows are closed on their own shelf.*

**`%467` and `%468` CLOSED, folded whole** to one
[shelf](../../REDS/REDS-presence-where-the-claim-was-content-rows-467-468.md), every other pin row being
...
*Folded in the same pass: the `%451`-`%469` account, whose rows stand on their own shelf.*
...
[blind-to-half shelf](../../REDS/REDS-blind-to-half-its-subject-rows-451-469.md). The register
