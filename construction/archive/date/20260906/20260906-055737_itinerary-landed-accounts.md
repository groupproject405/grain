# ITINERARY landed accounts -- `20260906.055737`

*Folded whole from the live front of [`../ITINERARY.md`](../../../ITINERARY.md) on `20260906.055737`, to bring the
card back under `living_pin_max_bytes` as `%464` landed on it. Two finished accounts, kept word
for word: the nib that stopped being typed, and the day-shelf row order that fired three times in
one lap. Both describe tools that shipped, so the card keeps a pointer and the record keeps every
word.*
