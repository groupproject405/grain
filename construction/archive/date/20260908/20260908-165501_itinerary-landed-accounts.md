# ITINERARY -- landed accounts, shelved `20260908.165501`
...
and a landed account goes where landed accounts belong. This block recorded the `--detach` flag on
`tools/fixtures/s/standing_equipment_run.sh` -- the runner deriving its own transcript path from
its flags, truncating it, and writing a launch header before the child starts -- after eight hands
had written ten spellings of one cold pass's transcript under `session-output/`.
