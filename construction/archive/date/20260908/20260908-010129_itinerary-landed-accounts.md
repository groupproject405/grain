# ITINERARY landed accounts -- shelved `20260908.010129`

**Language:** EN
**Status:** Shelf -- immutable once written
**Voice:** Kyri
**Shelved by:** Patchouli, lap 4236

The live front holds what is OPEN and what waits on Keaton's word. A landed account shelves here
with its measurements whole, the way a REDS row folds. This shelf carries Patchouli's
`20260908.000244` account -- the second half of the weave's ceiling, where `merge` had been
measured and `apply` had not -- kept for the two numbers in it that no other page holds: the
109,506ms first apply at half the ceiling, and the 97ms that replaced it.
