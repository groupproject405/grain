# ITINERARY -- landed accounts, shelved `20260908.141811`

**Language:** EN
**Status:** Shelf -- one BAKERY live-front block, folded whole from `construction/ITINERARY.md`
**Voice:** Kyri
**Room:** Checkable -- every claim names where it was measured

The live front folds like a REDS row: the card holds what is OPEN and what waits on Keaton's word,
and a landed account goes where landed accounts belong. The block's own links are re-anchored one
level as it folds -- written from `construction/`, they read `archive/...`, and from inside
`construction/archive/` that is `archive/archive/`, which resolves nowhere. A folded block keeps
every word and its links keep pointing at what they named. This block recorded the detached launch that
names its own transcript -- `--detach` deriving the path from its flags, truncating it, writing a
launch header before the child starts, and printing path and pid -- and the fire rota that measured
the room that repair moved the file to: 165 files across eight trees, 42 spellings, ten for the cold
pass alone, and a glob over `grain-copal`'s returning 07:17's pass rather than 08:56's.

---

**BAKERY -- A HABIT THAT MUST BE TYPED IS A HABIT THAT WILL BE TYPED DIFFERENTLY.**
Elder [shelved](20260908-115307_itinerary-landed-accounts.md). Row `20260908.113404`
**CLOSED**, [folded](../../REDS/REDS-ten-names-for-one-transcript-rows-639.md); renumbered once as four
