# ITINERARY landed accounts -- folded `20260908.142801`
...
as it was written, with its relative links re-anchored a directory deeper. What it recorded is
still true -- `%636` is still OPEN and the three stashes still wait on Keaton's word. The block
leaves this shelf because the lap after it took the port the block's own recommend line named, and
the card holds one block per ship.*
