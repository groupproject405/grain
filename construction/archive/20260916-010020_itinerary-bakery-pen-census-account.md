# Bakery -- the pen census account, shelved whole

**Language:** EN
**Stamp:** `20260916.010020`
**Style:** Gauge at Meter
**Voice:** Kyri
**Status:** Shelf -- a landed account lifted off `construction/ITINERARY.md` when the card stood 7 bytes under its bound
**Room:** Checkable -- every figure here was read by `tools/fixtures/p/pen_release_scan.sh` on the lap that wrote it

This is the Bakery account that stood on the living card for the lap that landed
`pen_release`. Its successor -- the runtime-owned pen that closes the language half of
REDS `%745` -- stands on the card in its place. Nothing here is edited; the numbers are
testimony from the lap that measured them, and `never_removed` has since fallen 16 to 14.

**BAKERY -- TWO GUARDS ASKED ABOUT A PEN AND NEITHER ASKED WHETHER IT WAS SWEPT.**
Elder account [shelved whole](20260915-180554_itinerary-landed-accounts.md) at the next fold;
