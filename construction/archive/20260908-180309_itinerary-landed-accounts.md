# ITINERARY -- landed accounts, shelved `20260908.180309`
...
and a landed account goes where landed accounts belong. This block recorded the Mantra CLI's diff
table taking `mantra/src/diff.rye`'s bounds -- `main.rye` calling `diff_mod.diff` and
`diff_mod.split_lines` rather than allocating `(m + 1) * (n + 1)` cells from two caller lengths
with no named max -- and the `seed_link` repair that let the public front door reach
`context/BHAKTA_STYLE.md`.

**BAKERY -- A PEER PUBLISHED MY LAP'S WORK 36 MINUTES BEFORE MY COMMIT.**
Elder [shelved](20260908-165501_itinerary-landed-accounts.md).
