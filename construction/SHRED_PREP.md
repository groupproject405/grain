...
### Class B -- the elder repository name, breached rather than shed (`20260908.180000`)

**A breach renames the living and keeps the elder in history; nothing here is cut.** On Keaton's
word, `veganreyklah2` leaves the living operational surface for **grain**. Measured first: **602
occurrences across 243 tracked files**, 187 of them dated testimony that keeps every word.

| Breached | To | Proven by |
| --- | --- | --- |
| six shared `/tmp/veganreyklah2-*` sentinels, five Caravan poll witnesses and one Mantra stop witness | per-run `mktemp -d /tmp/grain_poll.XXXXXX` | six GREEN on metal; `shared_pen` 47 -> 41 files, 13 -> 7 wipers |
| `caravan/capabilities.rye` paths `/home/xy/veganreyklah2` | `/home/xy/grain` | `caravan_capabilities_witness` GREEN |
| `context/specs/zig-0.16.0-local-setup.md` `~/veganreyklah2` | `~/grain` | the path it named was already wrong for every clone |
...
[`archive/20260908-180000_shred-prep-class-w-walked-and-kept.md`](archive/20260908-180000_shred-prep-class-w-walked-and-kept.md).
A walk whose verdict was KEEP is a decision already taken.*

## Completed QA queues

The two closed queues stand whole on [their shelf](archive/20260909-035800_shred-prep-closed-queues.md).
Their readings and decisions remain available; neither queues a new cut.

### Class M -- the table-store reading, prepped `20260909.035800`
