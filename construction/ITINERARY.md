...
- **Next doors, after that one (`20260821.203103`).** The Caravan ladder work is real and unfinished; it continues as **`lift`** rather than `fold`, and each lap is marked by **stamp and name** -- never `fold AJ`, and the total is counted with `git log --oneline --grep | wc -l` rather than carried in a name. Beside it: the **Caravan carry meter now reads two numbers** (`carry` and `CARRY_DELEGATES`, 2,365), so a lap that trades lines for delegates shows it. **Chapter G open doors** stand as the reach round left them -- a root-task skeleton on the vendored `libsel4`, Aurora on RISC-V under QEMU. And two named-and-held: the `manual/` rooms beyond the five guides read this day, and any page whose claim is about **behavior** rather than a name or a path, which only a reader's walk on a colder tree will test.
...
*One-page carry card for outer terminal - phone - waymarks. Refreshed when Keaton says **remember**. Debrided to the Compass Chapter `20260809.024320` -- the old season marks are gone; their greens live in the code and in the dated counsel.*

**Git nib:** `beca87bf59` (**Krittika leads, the room reads as plain ASCII, and the design room gets a map.** The nakshatra roster counts from **Krittika** to **Bharani** -- the Taittiriya order the note already cited -- with `seat_nakshatra.rye` rotated in the same commit and GREEN on metal. All **102** astrology files converted to 7-bit ASCII, diacritics by the IAST map the module's own string literals had been using for months, and the one glyph template kept as HTML numeric references so it renders unchanged. `active-designing/` gains a **walking index** over thirty-nine documents in seven sections -- one page rather than seven, since the smallest cluster holds two. Writing it found `readme_reach`'s ceiling aimed at the wrong number: reaching more documents raised the testimony count, so the ratchet punished opening a door; removed, with the LOST duty left where it already lives. Gauge gains **compare by what keeps, never by what decays**.)
...
calls `bash tools/ag/agent-jail.sh claude`, and `tools/l/launch-claude-chapter.rish` prints the
...
module name `Oven` and ten non-ASCII lines. **REDS %172** -- `phantom_path` answered red on a sound
