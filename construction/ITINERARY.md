...
**BAKERY -- THE NIB ALIAS CONTROL SHARES ONE GIT HISTORY.** The two write aliases
start from copies of one committed fixture. A forced one-second clock change
reproduces the elder failure; all 41 checks pass after the repair, including
the wrong-alias mutation. The nib-writer witness passes. Full cold at
`266b9aefa9`: 333 green, 41 red, 3 gated; `tree_moved=no`, 4,772 seconds.
Full hot verification and main integration remain owed; the repair package
is prepared for a review branch before the active 5,400-second timeout.
[Prior account](archive/20260925-051430_bakery-cold-recovery-account.md).
...
**Git nib:** `266b9aefa9` -- this commit's parent, resolvable everywhere (%401).
