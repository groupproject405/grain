...
Opcode 10 needed a genuinely new primitive the macro layer named honestly and left open: `#[axis value tree]`, the constructive counterpart to `slot()`'s own tree addressing. Same bit-walk, same address convention. Where `slot()` only reads, `edit()` records each level's untouched sibling on the way down and rebuilds the whole tree on the way back up, splicing the new value in at exactly the addressed leaf and leaving every other leaf as it was. Bounded by a `u64` axis's own width alone (at most 64 levels can ever exist), so that width is the one ceiling and two plain loops carry it -- iteration in place of recursion.
...
Per the runes-scoping study's own jet proposal, [`JET_DEC.md`](JET_DEC.md) names the first real jet: `dec` (decrement), a hand-built self-recursive Nock core proven identical to its native fast counterpart. A real dispatch mechanism recognizes one hint tag and falls through correctly on any other. The jet's own correctness obligation -- fast and slow always agree -- is proven directly rather than assumed; the interpreter's own honest depth limit is measured and named rather than hidden.
...
*May the primitive core stay small enough to read in one sitting, and may every opcode this interpreter ever grows keep the same bound its first twelve already prove: checked, bounded by a plain loop where one will do, and shown to reduce to what came before rather than trusted on its word.*
