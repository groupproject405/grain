...
| `ww` | GitHub `grain-ww/grain` | **The second door to the same projection** (Keaton's word `20260828`, on creating the `grain-ww` organization -- the name matches the living domain `grain-ww.com`). `publish-seed.sh` force-pushes the one crashed-wave commit to **both** doors; `ww` and `seed` are peer names for one seed, never two seeds. |

**Pier write ruling -- Option 2 SEATED** (`20260730.034136` - Keaton's word - red 32 - counsel lean held): ls-remote proves existence, never authorization. **auth-refused** (403) is named in the send-word guard. **Cloud** pushes `xy` branch + PR, and **FF-merges to `xy/main` itself** (approved `20260730.034520` -- Keaton's word: merge here in the Cloud environment). **`origin` / `main` syncs from the home pier** after Keaton pulls and GPG-pushes -- no home credential leaves home. Dual-send still *aims* both waters; when Cloud meets auth-refused on origin, name it and continue the `xykj61` path -- dual-send incomplete until the home pier closes the gap.
