...
**The loops' own state is a closed stack, by shape and by body.** Every agent loop keeps runtime
state -- today in six gitignored root directories (`.mind-state/`, `.dream-state/`, `.cursor-state/`,
`.zed-state/`, `.claude-state/`, `.cursor-agent-state/`), proposed to become one `loops/` room with a
subdirectory per body in
