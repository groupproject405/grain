...
Running `crypto_suite_witness` as a *load* rather than as a *proof* -- reading its closing line only for a wall-clock number -- surfaced a red the tree had carried unseen. The suite ended every run with *"all seventy-four Chapter G crypto files ... the count guard confirms the file count."* The count guard, run minutes earlier, had said **81**. `ls crypto/*.rye` shows **86**, of which five are symlink shims the guard correctly excludes.
