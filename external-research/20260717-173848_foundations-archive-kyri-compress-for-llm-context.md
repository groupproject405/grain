# Foundations Archive, Kyri Compress, and LLM Context Budget

**Language:** EN
**Version:** `20260717.173848` (Eastern)
**Style:** Gauge (see `../context/GAUGE_STYLE.md`)
**Voice:** Quin
**Status:** research for understanding -- archive vs compress - Kyri vs Markdown - LLM context (Two Rooms). Addressed to a generic Acme Corporation employee.
...
2. Compress those essays into succinct Kyri (`.kyri`) so LLM sessions spend fewer tokens.
...
| Rewrite dated foundations into Kyri essays | **Decline** -- dated_guard - accrete-never-break - teacher prose loses Radiant voice |
| Replace foundations with Kyri | **Decline** -- Kyri is seam notation, not essay form |
| Extend the seated **docs compression** layer | **Approve** -- already law; missing a compass / day-one page |
| Add a tiny **compass card** (Kyri pointers only) | **Approve** -- agent cold-start order without loading essays |
...
| Kyri notation | ~half the tokens of JSON for **config and seam values** -- not a substitute for why-prose |
...
## Why Kyri Essays Would Fail Here

Kyri's strength is **immutable key-value at a seam**: session logs, host cards, policy descriptors. Turning `the-happy-zone-and-the-thin-edge.md` into Kyri fields would:
...
**Kyri for LLM savings already works where it belongs:** load `GLOW_HOST.kyri`, a session log, or a **compass card** of paths -- not 500-line vision essays.
...
3. **Load a Kyri compass card** -- a few dozen tokens of `read <path>` lines for cold-start order.
...
| `docs/compass_card.kyri` | Succinct Kyri pointer list -- agent cold-start; no essay body |
...
Horizon (needs a later word): a `docs/TAME.md` thin compressor if `TAME_GUIDANCE.md` cold-loads too often; a `docs/LEXICON_CORE.kyri` of the twenty most-used terms -- only if measured pain appears.

## What This Does Not Claim

- It does not move any foundation into `archive/`.
- It does not convert essays to Kyri.
...
- `../active-designing/yonder/20260621-063912_kyri-notation.md`
