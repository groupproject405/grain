...
**Ground:** The Compass Chapter, JARL settlement complete at five transitions (`settlement/constellation.rye`). This paper opens the two horizons that follow — a constellation whose *shape* is chosen rather than fixed, and the Pond that loads it.
...
5. **The breach** — a next season, seated after the current Compass Chapter's rounds, whose spine is Pond-as-application-module and the loadable topologies it serves.
