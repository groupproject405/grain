...
**Held to the directory by** [`../tools/i/image_module_roster_witness.rish`](../tools/i/image_module_roster_witness.rish) over [`../tools/fixtures/m/module_roster_scan.sh`](../tools/fixtures/m/module_roster_scan.sh), which gates `unrostered`, `phantom`, `duplicate_rows`, and `mismatched_rows` at zero.
...
| [`text_reel.rye`](text_reel.rye) | Chapter G's capstone |
...
| Module | What it does |
|---|---|
| [`frames.rye`](frames.rye) | the open intra-frame video container (gfv1, Chapter G, the rung after the glyph atlas) |
| [`frames_delta.rye`](frames_delta.rye) | the open inter-frame video codec (gfd1, Chapter G, the rung after the gfv1 container) |
...
Chapter G's morphology and shape family: threshold, label, measure, thin, and describe what a mask holds.
