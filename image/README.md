...
This module is the floor beneath Chapter A's **parts marketplace** and its **Photos app**. It answers the plainest question a device family can ask — *can the person who holds it also hold every picture on it?* — with yes, proven on metal.
...
### Open Media Primitives — the color and font floor beside the codec (Chapter G)

The `image/` directory also holds the opening rungs of **Chapter G (Open Media Primitives)** — the open, bounded, permissively-licensed media family the image codec began. They finish for color and type what QOI finished for pixels: a value held exactly, every reading of it deterministic, every malformed input refusing by name.
...
The next Chapter G rungs are permissive font corpora and an intra-frame video container (QOI-per-frame), each its own round.
...
These wait for their own rounds (some for a web-search research pass, per the Chapter-A capture):
