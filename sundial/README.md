...
**Status:** Checkable -- sourced module health readings
...
point it at sourced module readings and it shows the tree's state at a glance. Each reading
carries its unit, date, and witness source, so a green or red result says where it came from.
...
Add a module reading with `add(name, green, unit, date, source)`. An empty source is refused as
`SourcelessReading`. Once admitted, the Sundial answers:
...
member vault green unit witness-status date 20260829 source vault-split
