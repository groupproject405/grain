# STOA -- the Glow SLC ladder: rune heads, nest, product

*Waymark for the Glow language ladder: rune heads, nest and type, the lowering to Rye, and the
product surfaces built on them. Compresses the silo; never replaces it.*

**Status:** Checkable -- operator compressor (STOA0-346 GREEN; STOA347 onward marked by stamp and name)
**Depth:** guide
**Ceiling:** <=80 lines
...
argv family, the `shape-*` dual desks, the HAWM harvest chain -- takes one row here and leaves the
steps to the witness that runs them. Rungs that recorded a lean rather than a claim are counted in
their family's range: 51 of them, each saying *A send, B rest available*, which the logs and the
witnesses already keep.
...
| **STOA106-121** | Nest/type - barket tokenize-to-lower - nesting-nests wire | nest - barket - bartis | **GREEN** `042931`...`182915` |
| **STOA122-133** | Named allowlist amount-to-pair - flips - amount/count argv | nest - barket - glow_run | **GREEN** `183259`...`194143` |
| **STOA134-147** | Triple-to-nona `$:` ladder - flips - capacity freeze at 9 | nest - barket - bartis | **GREEN** `194437`...`231419` |
| **STOA148-160** | amount...nona-shape Grain aliases | `glow_nest_type_witness` - desks | **GREEN** `234340`...`005327` |
| **STOA161-176** | Thirteen `shape-*` dual desks, amount through nona - living-pin polish - multi-field named cast and peek widen | `glow_run` desks - named-cast witness | **GREEN** `010211`...`014114` |
| **STOA177-198** | Seva B0 Glow: thin `@t` cord and `@ux` hex auras and their lower emit - `$%` payload - ShapeSpec/rune_shape/lower_shape API - claim shell - digest, append, replay thin ports - fold product surface | `glow_seva_b0_*` witnesses - `glow_lower_shape` - `glow_run` | **GREEN** `165342`...`183554` |
| **STOA199A-214** | Pixel Glow-fold eyes - sign/verify surface - log-line format/parse - fact-line shape desk and cast twins - cord bound 1024 - FactFields parse | `glow_seva_b0_sign/line/fields/replay` - fold | **GREEN** `185805`...`194756` |
| **STOA215-240** | Glow source literals: cord and hex literal lex - `face=lit` parse and lower emit - fixture lit desks - demo line emit from seated pins - append events 1-3 - claim/fold desk-file I/O - NativeActivity embedded lit desk | `glow_tokens_witness` - `glow_face_lit_witness` - `glow_lower_face_lit_witness` - append/line witnesses | **GREEN** `194941`...`20260722.135605` |
...
| **STOA263-273** | Pixel authored-fold eyes - Dexter-to-Seva typed memo `append_event` - Dexter submit helper - NativeActivity submit to live root - live-root Skate paint | `glow_seva_b0_typed_append_witness` - `dexter_seva_append_witness` - `seva_live_root_glass_witness` | **GREEN** `20260722.150432`...`152217` |
| **STOA274-291** | HAWM 1A: log-harvest survey - boot-to-harvest chain host GREEN - emulator serial pin - harvest `session_root` - hawm0 stopped season fold - Pixel LIVE eyes | `hawm_log_harvest_witness` - `hawm0_emulator_serial_pin_witness` - chain witness onpath | **GREEN** `20260722.152404`...`155130` |
...
| **STOA337** | `?: (eq face N)` cond body: exact match decides where `gth` cannot name the edge | `glow_lower_shop_gate_witness` - welcome and argv bodies | **GREEN** `20260802.211829` |
...
now ride `glow_choir_witness` on the cadence tier of `construction/standing-equipment.kyri`.
Sibling ladders: **ZETA** - **LULU** - **SETU** - **TUBE**.
