...
Brix is Grain's **composition language** — it declares systems and evaluates to **Bron** (plain key-value data, one field per line). A `.brix` descriptor names a shape: scalar keys and a repeatable `field` list (see `../context/baton-museum/*.brix`, `../tools/gen/chapter/recursion_block.brix`, the root `../.brix`).
