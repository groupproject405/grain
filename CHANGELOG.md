...
Grain is in its crashed-wave phase -- the seed has landed in public ground -- and has not yet cut a released version. When it does, the release tags will land here as anchors into the commit history, so a version reads as a range of the record already kept -- never as a summary that replaces it.
