...
**Style:** Gauge, **Meter** setting -- this is a ledger of findings, and a finding is most often a
refusal: the field that did not exist, the contract that did not hold. Meter is the setting whose
subject is refusal, so its negation share is reported rather than capped. Graded at Field the book
reads C+ on register alone; graded at the setting it is written in, A. Declared here on
`20260910.042550` because the page carried no Style line at all, and a grader with no declaration
guesses (`.claude/rules/quality-assurance.md`).
...
**Four chapters x sixteen entries = sixty-four findings**, one chapter per equinox of the season that fills it. The bound is the book's own: at sixty-four, the almanac closes and a successor is chartered, exactly as seasons close. Chapters carry no themes in advance -- findings arrive in the order the code runs, and the order is part of the record.
...
### 86. IronBeetle ep031.5 keeps a durable fact in one coherent form; journal and checkpoint must truly copy the same thing.
...
### 92. IronBeetle ep037.5 folds compaction into each commit: garbage collection at allocation so replicas stay byte-identical.
