...
**Language:** EN  
**Last updated:** 2026-07-28 (`235520` — Oven Chapter o1 path cut)  
...
Oven names training and fine-tuning on our own corpus (was **Anvil** forge path). The catalog folds `.rye` / `.rish` chunks from **our tree** — gratitude and vendor stay in their reading rooms. Query filters by kind and path prefix. Corpus views fold those hits onto Skate.
...
| [`oven.rye`](oven.rye) | Selftest |
| [`fixtures/rye_corpus.bron`](fixtures/rye_corpus.bron) | Pinned chunk list |
| `oven.peal` | The forge rings once — Opus-in-Ogg under `.peal`. Gratitude [`../gratitude/OpusOggXiph.md`](../gratitude/OpusOggXiph.md). |
...
*May the oven know its own tree. May LoRA wait until the catalog tells the truth.*
